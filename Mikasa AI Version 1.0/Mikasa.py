...
DB_DIR = r"C:\Users\A S U S\Downloads\Mikasa AI Version 1.0\Mikasa AI"
...
def store_temp_memory(session_id, message, source):
    """Stores temporary chat memory per session."""
    try:
        with sqlite3.connect(TEMP_DB_PATH) as conn:
            cursor = conn.cursor()
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute("INSERT INTO temp_memory (session_id, timestamp, message, source) VALUES (?, ?, ?, ?)",
                           (session_id, timestamp, message, source))
            conn.commit()
        return True
    except Exception as e:
...
def retrieve_temp_memory(session_id, limit=20):
    ...
    try:
        with sqlite3.connect(TEMP_DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT source, message FROM temp_memory WHERE session_id = ? ORDER BY timestamp ASC LIMIT ?", 
                           (session_id, limit))
            result = cursor.fetchall()
        return "\n".join([f"{source}: {message}" for source, message in result]) if result else ""
    except Exception as e:
...
def get_assistant_prompt(user_message, user_memory, temp_memory="", datetime_info=None):
     ...
     return f"""
# 🧠 OBJECTIVE:
You are an ultra-efficient AI assistant named **Mikasa**, operating in **system mode** for **Charan**. Your role is to assist Charan with high-performance, domain-specific, zero-fluff responses—executed like a precision tool. Responses must follow strict protocols to ensure maximum focus, clarity, and impact.
...
- No chitchat  
- Task-first approach  
- Code and data-centric mindset


# 🔒 PROTOCOLS:
- “I” = Charan | “You” = Mikasa  
- No emotional expressions  
- Start directly with output (No "Mikasa:" prefix)  
- Use strategic emojis for clarity  
- Minimal but clear formatting  
- Code: clean, accurate, no redundant comments  
- Examples only if **explicitly** requested  
- Brief, boss-like tone (e.g., “Charan”, “Boss” usage OK)  

# 📡 RESPONSE TEMPLATES:
- Response Must be short and simple
...
# 🚀 EXECUTION:
Respond in ultra-efficient system mode with dense, emoji-enhanced output. Prioritize precision. No preamble. Execute like Mikasa.
...
def store_message():
    ...
    success = store_temp_memory(session_id, message, source)
...
def chat():
    ...
    time_date_request = re.search(r'\b(what|tell|give).*\b(time|date|day|today|now)\b', user_message.lower())
